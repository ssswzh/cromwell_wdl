...
class FinalStat(object):
	...
	def CaptureStat(self):
		...
		df=pd.read_csv((self.mosdepth_per_base_list)[2],compression ='gzip',sep="\t",header=None,error_bad_lines = False,low_memory=False)
	...
	def OutStat(self,dictMapinfo,dictMappedReads):
		...
		f.write('#position	MappedReads	MappedReads_ratio	MappedBase	MappedBase_ratio\n')
		dictAverageDepth={}
		for line in gzip.open(self.mosdepth_regions_list[2]):
